...
- [x] Настройка базового проекта Django.
- [x] Главная страница (`/`) — лента блогов с последней опубликованной статьёй каждого
- [ ] Статическая страница "Обо сайте" (`/about/`).
- [x] Приложение `blog`:
    - [x] Модели Blog и Article с поддержкой draft/published.
    - [x] Отображение только опубликованных статей
    - [x] Детальная страница для каждого блога (`/blog/<id>/`/`).
    - [x] Кликабельные заголовки блогов
- [x] Полное покрытие юнит-тестами
- [x] Функциональные тесты на Selenium (браузерные)
- [x] Базовая HTML-разметка с помощью шаблонов Django.
- [x] Минималистичный CSS.
